...
// 20분에 한번 실행
properties([pipelineTriggers([cron('H/5 02-23 * * *')])])

node {
    stage("Chkstat1 Action") {
        try {
            def baseUrl = "http://wapi.10x10.co.kr"
            def response = httpRequest url: baseUrl + "/outmall/batch/batchproc.asp?mallid=lfmall&apiAction=CHKSTAT&topN=100",
            ...
            if (responseText != ""){
                ...
                for(i=0; i < responseTextArr.length; i++){
                    ...
                    def responseProc = httpRequest url: baseUrl + "/outmall/proc/lfmallProc.asp?redSsnKey=system&jenkinsBatchYn=Y&mallid=lfmall&action=CHKSTAT&idx="+idxArr+"&itemid="+itemArr,
                                            validResponseCodes: "200:499"
