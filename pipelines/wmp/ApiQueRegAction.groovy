...
node {
    stage("REG Action") {
        try {
            ...
            def response = httpRequest url: baseUrl + "/outmall/batch/batchproc.asp?mallid=WMP&apiAction=REG&topN=100",
            ...
            if (responseText != ""){
                ...
                for(i=0; i < responseTextArr.length; i++){
                    ...
                    def responseProc = httpRequest url: baseUrl + "/outmall/proc/wmpProc.asp?redSsnKey=system&jenkinsBatchYn=Y&mallid=WMP&action=REG&idx="+idxArr+"&itemid="+itemArr,
                                            validResponseCodes: "200:499"
