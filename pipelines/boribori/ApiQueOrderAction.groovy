...
node {
    stage("OrderNewSelect Action") {
        try {
            ...
            def response = httpRequest url: baseUrl + "/admin/etc/order/xSiteOrder_Ins_Process.asp?sellsite=boribori1010&gubunCode=c",
                                    timeout:6000,
    ...
    stage("OrderInput Action") {
        try {
            ...
            def response = httpRequest url: baseUrl + "/outmall/shintvshopping/orderAutoInput.asp?sellsite=boribori1010",
                                    timeout:6000,
                                    validResponseCodes: "200:599"
            println("Status: "+ response.status)
            println("Content: "+ response.content)
