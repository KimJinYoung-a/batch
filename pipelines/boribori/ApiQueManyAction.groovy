...
node {
    stage("SOLDOUT Action") {
        try {
            ...
            def response = httpRequest url: baseUrl + "/thirdparty/internal/queue/read?mallId=boribori1010&apiAction=SOLDOUT&topCount=30"
    ...
    stage("Price Action") {
        try {
            def baseUrl = "http://gateway.10x10.co.kr"
            def response = httpRequest url: baseUrl + "/thirdparty/internal/queue/read?mallId=boribori1010&apiAction=PRICE&topCount=100"
            def map = parseJsonToMap(response.content)
            if (map.result != ""){
                def jsonresultArr = map.result.split(",")
                for(i=0; i < jsonresultArr.length; i++){
                    ...
                    def idxArr = jsonresultArr[i].split("##")[1]
                    try {
                        def responseProc = httpRequest url: baseUrl + "/external/apis/product/update/price/batch?mallId=boribori1010&action=PRICE&idx="+idxArr+"&itemId="+itemArr+"&user=batch",
                                            validResponseCodes: "200:599"
                        println("Status: "+ responseProc.status)
                        println("Content: "+ responseProc.content)
                        if (responseProc.status > 400) {
