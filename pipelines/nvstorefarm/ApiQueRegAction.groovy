...
// 20분에 한번 실행 (본격적으로 등록할 때 10분마다였음..2022-09-14)
...
node {
    stage("Image Action") {
        try {
            ...
            def response = httpRequest url: baseUrl + "/outmall/batch/batchproc.asp?mallid=nvstorefarm&apiAction=Image&topN=100",
    ...
    stage("Reg Action") {
        try {
            def baseUrl = "http://wapi.10x10.co.kr"
            def response = httpRequest url: baseUrl + "/outmall/batch/batchproc.asp?mallid=nvstorefarm&apiAction=REG&topN=100",
                                    validResponseCodes: "200:499"
            def responseText = response.content
            if (responseText != ""){
                def responseTextArr = responseText.split(",")
