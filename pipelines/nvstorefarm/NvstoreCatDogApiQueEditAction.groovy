...
node {
    ...
    stage("Edit Action") {
        try {
            def baseUrl = "http://wapi.10x10.co.kr"
            def response = httpRequest url: baseUrl + "/outmall/batch/batchproc.asp?mallid=Mylittlewhoopee&apiAction=EDIT&topN=50",
                                    validResponseCodes: "200:499"
            def responseText = response.content
            if (responseText != ""){
                def responseTextArr = responseText.split(",")
    ...
    stage("SellChg Action") {
        try {
            def baseUrl = "http://wapi.10x10.co.kr"
            def response = httpRequest url: baseUrl + "/outmall/batch/batchproc.asp?mallid=Mylittlewhoopee&apiAction=EDIT2&topN=100",
                                    validResponseCodes: "200:499"
            def responseText = response.content
            if (responseText != ""){
                def responseTextArr = responseText.split(",")
    ...
    stage("ChkOpt Action") {
        try {
            def baseUrl = "http://wapi.10x10.co.kr"
            def response = httpRequest url: baseUrl + "/outmall/batch/batchproc.asp?mallid=Mylittlewhoopee&apiAction=CHKOPT&topN=70",
                                    validResponseCodes: "200:499"
            def responseText = response.content
            if (responseText != ""){
                def responseTextArr = responseText.split(",")
                for(i=0; i < responseTextArr.length; i++){
                    ...
                    def responseProc = httpRequest url: baseUrl + "/outmall/proc/MylittlewhoopeeProc.asp?redSsnKey=system&jenkinsBatchYn=Y&mallid=Mylittlewhoopee&action=CHKOPT&idx="+idxArr+"&itemid="+itemArr,
                                            validResponseCodes: "200:499"
