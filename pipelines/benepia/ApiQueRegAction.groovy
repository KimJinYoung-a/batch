...
node {
    stage("REG Action") {
        try {
            ...
            def response = httpRequest url: baseUrl + "/thirdparty/internal/queue/read?mallId=benepia1010&apiAction=REG&topCount=100"
            ...
            if (map.result != ""){
                ...
                for(i=0; i < jsonresultArr.length; i++){
                    ...
                    try {
                        def responseProc = httpRequest url: baseUrl + "/external/apis/product/create/batch?adminId=system&mallId=benepia1010&action=REG&idx="+idxArr+"&itemId="+itemArr+"&user=batch",
