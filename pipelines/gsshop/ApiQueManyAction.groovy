...
node {
    ...
    stage("Price Action") {
        try {
            def baseUrl = "http://wapi.10x10.co.kr"
            def response = httpRequest url: baseUrl + "/outmall/batch/batchproc.asp?mallid=gsshop&apiAction=PRICE&topN=100",
                                    validResponseCodes: "200:499"
            def responseText = response.content
            if (responseText != ""){
                def responseTextArr = responseText.split(",")
                for(i=0; i < responseTextArr.length; i++){
                    ...
                    def responseProc = httpRequest url: baseUrl + "/outmall/proc/GSShopProc.asp?redSsnKey=system&jenkinsBatchYn=Y&mallid=gsshop&action=PRICE&idx="+idxArr+"&itemid="+itemArr,
                                            validResponseCodes: "200:499"
    ...
    stage("Edit1 Action") {
        try {
            def baseUrl = "http://wapi.10x10.co.kr"
            def response = httpRequest url: baseUrl + "/outmall/batch/batchproc.asp?mallid=gsshop&apiAction=EDIT&topN=80",
                                    validResponseCodes: "200:499"
            def responseText = response.content
            if (responseText != ""){
                def responseTextArr = responseText.split(",")
